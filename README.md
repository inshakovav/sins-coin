...
-------
какие ресурсы системы
Какие нам дадут
	RAM
	Volume
Интеграции какие, брокеры

Внутрибансковский:
	какие права
	какие ТУЗ, с какими правами

Если в общем кластере дают свой namespace и там что хотиле делаете, то норм.
Если он нас устраивает

Как происходит процесс релиза, деплоя
Как дожно быть, что не так

Инструменты DevOps банка
	как деплой организован

Где храниться код пайта
Что выполняется по заявками
Чтобы показал релизный цикл.
Свой кластер:
	Серверы свои
	Нужны ОПСы
...
Настройка кластера это не работа Девопса
--
